...
import { useLLMRouters } from "../application/useSettings";
import { Card, CardHeader, CardTitle, CardContent } from "@/shared/ui/ui/Card";
import { Badge } from "@/shared/ui/ui/Badge";
import { Route, Settings, Play, ShieldCheck, BarChart2 } from "lucide-react";
    ...
    if (isLoading) {
        return (
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {[1, 2].map((index) => <Skeleton key={index} className="h-48 w-full" />)}
            </div>
        );
    }
