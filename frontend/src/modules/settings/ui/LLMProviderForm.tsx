...
import { z } from "zod";
import { LLMProviderSchema } from "@/shared/domain/settings";
import { useCreateLLMProvider } from "../application/useLLMProviders";
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
export const LLMProviderForm = ({
    ...
    const form = useForm<FormData>({
        resolver: zodResolver(CreateProviderFormSchema) as unknown as Resolver<FormData>,
        defaultValues: {
            provider_name: "",
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <Select onValueChange={(val) => field.onChange(val === "true")} defaultValue={String(field.value)}>
                                            ...
                                            <SelectContent>
                                                <SelectItem value="true">Tier 1 (High Reliability)</SelectItem>
                                                <SelectItem value="false">Tier 2 (Batch/Economic)</SelectItem>
                                            </SelectContent>
                                        </Select>
                                        <FormMessage />
                                    </FormItem>
