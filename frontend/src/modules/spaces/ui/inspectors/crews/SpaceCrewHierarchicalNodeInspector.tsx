    ...
    return (
        <SpaceInspectorPanel>
                    ...
                    <div className="h-[calc(100vh-192px)] pb-40">
                        <SpaceCrewOrchestrationLayout>
                            <div className="space-y-10">
                                ...
                                {(isDone || isAborted) && (
                                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex items-center gap-3 py-4 border-b border-zinc-900" >
                                        {isDone ? <CheckCircle2 size={24} className="text-white" /> : <CircleStop size={24} className="text-zinc-500" />}
                                        <h3 className="text-sm font-black text-white tracking-tight uppercase">{isDone ? "Hierarchy Done" : "Hierarchy Stopped"}</h3>
                                    </motion.div>
                                )}
                            </div>
                        </SpaceCrewOrchestrationLayout>
            ...
            <SpaceInspectorFooter>
                ...
                {(isDone || isAborted) && (
                    <Button size="sm" className="w-full bg-zinc-200 text-black font-black uppercase text-[10px] rounded-md h-10 hover:bg-white" onPress={() => transitionTo('missing_context', { tasks: [] })}>Restart Team</Button>
                )}
            </SpaceInspectorFooter>
        </SpaceInspectorPanel>
    );
