                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        {tasks.map((task, i) => (
                                            <motion.div 
                                                ...
                                                className="p-4 bg-zinc-900/30 border border-zinc-800 rounded-xl space-y-3"
                                            >
                                                <div className="flex items-center gap-2">
                                            ...
                                            {tasks.map((task) => {
                                                ...
                                                return (
                                                    <motion.div 
                                                        key={task.id} 
                                                        layout
                                                        className={cn("p-4 border border-zinc-800 rounded-xl space-y-2.5 transition-all duration-500", isAgentWorking ? "bg-white/[0.02] border-white/10" : "opacity-60")}
                                                    >
                                                        <div className="flex items-center justify-between">
                                                            <div className="flex items-center gap-2">
                                                                <div className="relative">
                                                                    <Avatar name={task.assignedAgentTitle} size="sm" className={cn("w-6 h-6 text-[8px] font-black transition-all", isAgentWorking ? "bg-white text-black shadow-[0_0_15px_rgba(255,255,255,0.3)]" : "bg-zinc-800 text-zinc-500")} />
                                                                    {isAgentWorking && (
                                                                        <motion.div layoutId="active-glow-p" className="absolute -inset-1 rounded-full border border-blue-400/30" animate={{ scale: [1, 1.2, 1], opacity: [0.5, 0, 0.5] }} transition={{ duration: 2, repeat: Infinity }} />
                                                                    )}
                                                                </div>
                                                        ...
                                                        <AnimatePresence mode="wait">
                                                            {task.status !== 'pending' && (
                                                                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
                                                                    <p className={cn("text-[10px] font-mono leading-relaxed italic transition-all pl-8", isAgentWorking ? "text-zinc-400" : "text-zinc-700")}>
                                                                        {task.thought}
                                                                    </p>
                                                                </motion.div>
                                                            )}
