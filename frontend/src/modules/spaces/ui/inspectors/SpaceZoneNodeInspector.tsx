                ...
                <Tab 
                    ...
                    title={
                        <div className="flex items-center gap-2">
                            <Box size={12}/> 
                            Artefacts
                            {childArtefacts.some(entry => entry.artefact.status === 'in_review') ? (
                                <div className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse shadow-[0_0_8px_rgba(59,130,246,0.8)]" />
                            ) : childArtefacts.length > 0 ? (
                    ...
                    <ScrollShadow className="h-[calc(100vh-192px)] p-8">
                        <div className="space-y-6">
                            <div className="space-y-3">
                                {childArtefacts.map((entry, index) => (
                                    <div 
                                    ...
                                    >
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                {entry.artefact.link && (
                                                    <ExternalLink size={10} className="text-zinc-600 group-hover:text-zinc-200 transition-colors" />
                                                )}
                                            </div>
                                        </div>
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className={cn(
                                                ...
                                                entry.artefact.status === 'approved' ? "text-green-500" : "text-blue-400"
                                            )}>
                                                {entry.artefact.status === 'approved' ? <CheckCircle size={10} /> : <Clock size={10} />}
                                                <span className="text-[8px] font-black uppercase">
                                                    {entry.artefact.status === 'approved' ? 'Approved' : 'In Review'}
                                                </span>
                                            </div>
                                        </div>
