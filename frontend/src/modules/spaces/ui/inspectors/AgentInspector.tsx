...
export const AgentInspector = ({ node, onNodeDataChange }: AgentInspectorProps) => {
    const [agentStatus, setAgentStatus] = useState(node.data.state || "pending");

    const handleStatusChange = useCallback((newStatus: string) => {
        setAgentStatus(newStatus);
        onNodeDataChange(node.id, { state: newStatus });
                        ...
                        <div className="space-y-2">
                            ...
                            >
                                {agentStatuses.map((status) => (
                                    <SelectItem key={status.key} value={status.key} className="text-xs font-mono text-zinc-400 uppercase data-[hover=true]:bg-zinc-900 data-[hover=true]:text-white">
                                        {status.label}
                                    </SelectItem>
                                ))}
                            </Select>
