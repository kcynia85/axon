...
interface ArchetypeIdentitySectionProps {
	readonly syncDraft?: () => void;
}

export const ArchetypeIdentitySection = ({ syncDraft }: ArchetypeIdentitySectionProps) => {
			...
			<div className="space-y-12">
				<FormField
					...
					render={({ field, fieldState }) => (
						<FormItemField label="Archetype Name" error={fieldState.error?.message}>
							<FormTextField 
								{...field} 
				...
				<FormField
					...
					render={({ field, fieldState }) => (
						<FormItemField label="Rola" error={fieldState.error?.message}>
							<FormTextField 
								{...field} 
								placeholder="np. Definiuje strategię produktu" 
								className="focus:placeholder:text-zinc-900 dark:focus:placeholder:text-white transition-colors"
								value={(field.value as string) || ""}
								onBlur={syncDraft}
							/>
