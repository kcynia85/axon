...
export const LivePoster = ({ data }: LivePosterProps) => {
  const modelName = data.llm_model_id ? MODEL_NAMES[data.llm_model_id] || data.llm_model_id : "Select Model";
        ...
        <div className="mt-6 space-y-3 px-2">
            ...
            <div className="flex gap-2">
               {data.reflexion && (
                 <div className="flex items-center gap-1 px-2 py-1 bg-primary/10 rounded-md border border-primary/20 animate-pulse">
                    ...
                    <span className="text-[7px] font-mono uppercase font-bold text-primary">Reflexion</span>
                 </div>
               )}
               {data.grounded_mode && (
                 <div className="flex items-center gap-1 px-2 py-1 bg-blue-500/10 rounded-md border border-blue-500/20">
                    ...
                    <span className="text-[7px] font-mono uppercase font-bold text-blue-500">Grounded</span>
                 </div>
               )}
            </div>
            ...
            <div className="flex justify-between items-center text-[9px] font-mono uppercase tracking-widest opacity-40">
                ...
                <span className="text-white font-bold text-[8px]">
                   {data.llm_model_id === "gemini-2.0-flash" ? "$0.0001" : "$0.002"} / 1k tkn
                </span>
            </div>
        </div>
      </div>
