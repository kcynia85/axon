...
class SemanticCache:
    ...
    def get_instance(cls):
        if cls._instance is None:
            # Check if we have a vector DB URL available before initializing
            if not settings.VECTOR_DATABASE_URL:
                # We could try to resolve from DB here, but for cache we might just skip it
                print("⚠️ Semantic Cache disabled: Missing VECTOR_DATABASE_URL in .env")
                return None
            cls._instance = cls()
