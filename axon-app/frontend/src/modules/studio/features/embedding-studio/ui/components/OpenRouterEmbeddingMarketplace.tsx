...
import { Search, Filter, Plus, X, Zap, BarChart3, AlertCircle, Info } from "lucide-react";
...
const FILTER_GROUPS: readonly FilterGroup[] = [
    ...
    },
    {
        ...
        type: "checkbox",
        options: [
            { id: "cheap", label: "Cheap (< $0.1/1M)", isChecked: false },
        ]
    }
];
...
export const OpenRouterEmbeddingMarketplace = ({ isOpen, onOpenChange, onSelect, installedModelId, models, isLoading }: Props) => {
    const [search, setSearch] = useState("");
    const [activeFilters, setActiveFilters] = useState<ActiveFilter[]>([]);
    const [pendingFilterIds, setPendingFilterIds] = useState<string[]>([]);
    const parentRef = React.useRef<HTMLDivElement>(null);

    // Heuristic to filter only embedding models
    const onlyEmbeddingModels = useMemo(() => {
    ...
    const filteredModels = useMemo(() => {
        return onlyEmbeddingModels.filter(model => {
            ...
            const matchesFilters = activeFilters.every(filter => {
                ...
                if (filterId === "cheap") return promptPrice < 0.10;
                return true;
            });

            return matchesSearch && matchesFilters;
        });
    }, [onlyEmbeddingModels, search, activeFilters]);
    ...
    }, [onlyEmbeddingModels, search, pendingFilterIds]);
                        ...
                        <div className="flex flex-col border-b border-zinc-800/50 bg-zinc-900/20 backdrop-blur-sm z-10 shrink-0">
                            <div className="flex items-center justify-between p-8 pb-6">
                                <div className="space-y-1">
                                    <DialogPrimitive.Title className="text-2xl font-bold tracking-tight text-white flex items-center gap-3">
                                        OpenRouter Embedding Marketplace
                            ...
                            <div className="px-8 pb-4 flex items-center gap-4">
                                <div className="relative flex-1 group">
                                    ...
                                    <Input 
                                        placeholder="Szukaj modeli (np. OpenAI, Voyage, Cohere)..." 
                                        value={search}
