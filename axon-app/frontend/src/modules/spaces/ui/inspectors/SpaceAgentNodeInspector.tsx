...
/**
 * SpaceAgentNodeInspector - Pure view component for agent node details.
 * Delegates logic to useSpaceAgentInspector hook.
 */
export const SpaceAgentNodeInspector = ({ data, nodeId, onPropertyChange }: SpaceAgentInspectorProperties) => {
    const { state, actions } = useSpaceAgentInspector(data, nodeId, onPropertyChange);
                                            ...
                                            {state.isDetailsOpen && (
                                                ...
                                                >
                                                    <div className="space-y-1.5">
                                                        {state.workingLogs.map((log: any, i: number) => {
                                                            const isLogDone = data.progress > log.threshold;
                                                            const isLogCurrent = data.progress >= log.threshold && (i === state.workingLogs.length - 1 || data.progress < state.workingLogs[i+1].threshold);
                                                            return (
                                                ...
                                                >
                                                    <div className="text-[10px] text-zinc-400 space-y-1 font-mono">
                                                        ...
                                                        <div className="pt-2 text-zinc-600 font-bold uppercase" suppressHydrationWarning>
                                                            Całkowity czas: {data.metrics?.duration || '4 min'} | Zużycie: {state.simulatedTokens.toLocaleString()} tokenów
                                                        </div>
                                                    </div>
                                                </motion.div>
                                            )}
                ...
                <SpaceInspectorFooter>
                    {state.isMissingContext && (
                        <div className="space-y-3">
                            ...
                            >
                                {data.progress > 0 ? "Wznów pracę" : (state.isContextComplete ? "Prepare Briefing" : `Missing context`)}
                            </Button>
                            {data.progress > 0 && (
                                <Button 
                                    size="sm" 
