...
import { Tabs, Tab, ScrollShadow, Avatar } from "@heroui/react";
...
export const SpaceCrewHierarchicalNodeInspector = ({ 
    ...
    const { state, actions } = useSpaceCrewHierarchicalInspector(data, nodeId, onPropertyChange);
                                ...
                                <SpaceCrewProgressBar progress={state.progressValue}>
                                    <div className="space-y-8">
                                        {state.tasks.map((task: any, i: number) => {
                                            ...
                                            return (
                                                <div key={task.id} className={cn("space-y-1.5", isAgentWorking ? "opacity-100" : "opacity-40")}>
                                                    <div className="flex items-center gap-2">
                                                        <Avatar name={task.assignedAgentTitle} size="sm" className="w-6 h-6 bg-zinc-800" />
                                                        <span className="text-[11px] font-black uppercase">{i+1}. {task.assignedAgentTitle}</span>
                                                    </div>
                                                    {task.thought && <p className="text-[10px] font-mono text-zinc-400 pl-8 italic">{task.thought}</p>}
                                                </div>
                                            );
                                        })}
                                    </div>
