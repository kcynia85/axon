...
import { Trash2, Settings, Link as LinkIcon, Key, HelpCircle, Server } from "lucide-react";
import { Badge } from "@/shared/ui/ui/Badge";
import { AutomationProviderUI } from "./AutomationProvidersBrowserView";

type AutomationProviderSidePeekProps = {
  readonly provider: AutomationProviderUI | null;
  readonly isOpen: boolean;
  readonly onClose: () => void;
  readonly onConfigure?: (provider: AutomationProviderUI) => void;
  readonly onDelete?: (id: string) => void;
}

export const AutomationProviderSidePeek = ({ provider, isOpen, onClose, onConfigure, onDelete }: AutomationProviderSidePeekProps) => {
      ...
      <div className="space-y-12">
        ...
        <div className="grid grid-cols-2 gap-4 pb-10 border-b border-muted">
          <div className="space-y-2">
            <div className="text-base font-bold text-muted-foreground">Typ Autoryzacji</div>
            <div className="text-base font-bold">{provider.authType}</div>
          </div>
          <div className="space-y-2">
            <div className="text-base font-bold text-muted-foreground">Platforma</div>
            ...
            {provider.baseUrl && (
              <div className="flex items-center justify-between p-3 rounded-lg bg-muted/30 border border-primary/5">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-base font-mono font-semibold truncate max-w-[200px]">{provider.baseUrl}</span>
                </div>
            ...
            <div className="flex items-center justify-between p-3 rounded-lg bg-muted/30 border border-primary/5">
              <div className="flex items-center gap-3">
                ...
                <span className="text-base font-mono font-semibold">
                  {provider.authType === "Brak" ? "Nie wymaga klucza" : "•••••••••••••••••••••••••"}
                </span>
              </div>
              <Badge variant="outline" className="text-xs h-5 px-2 py-0 font-bold opacity-50">SECRET</Badge>
            </div>
          </div>
        </section>
      </div>
